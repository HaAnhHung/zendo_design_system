// ignore_for_file: avoid_hard_coded_text_style
import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

import 'app_colors.dart';
import 'dimens/app_dimen.dart';
import 'dimens/dimens.dart';

/// AppTextStyle format as follows:
/// s[fontSize][fontWeight][Color]
/// Example: s18w400Primary

class AppTextStyles {
  AppTextStyles._();
  static const _defaultLetterSpacing = 0.03;

  static const _baseTextStyle = TextStyle(
    letterSpacing: _defaultLetterSpacing,
    // height: 1.0,
  );

  static TextStyle s14w400Primary({
    double? tablet,
    double? ultraTablet,
  }) =>
      GoogleFonts.plusJakartaSans(
        fontSize: Dimens.d14.responsive(tablet: tablet, ultraTablet: ultraTablet),
        fontWeight: FontWeight.w400,
        color: AppColors.current.primaryTextColor,
        letterSpacing: _defaultLetterSpacing,
      );

  static TextStyle s14w400Secondary({
    double? tablet,
    double? ultraTablet,
  }) =>
      GoogleFonts.plusJakartaSans(
        fontSize: Dimens.d14.responsive(tablet: tablet, ultraTablet: ultraTablet),
        fontWeight: FontWeight.w400,
        color: AppColors.current.secondaryTextColor,
        letterSpacing: _defaultLetterSpacing,
      );

  // Manrope for Display & Headlines
  static TextStyle displayLarge({
    double? tablet,
    double? ultraTablet,
  }) =>
      GoogleFonts.manrope(
        fontSize: Dimens.d32.responsive(tablet: tablet, ultraTablet: ultraTablet),
        fontWeight: FontWeight.w600,
        color: AppColors.current.primaryTextColor,
        letterSpacing: -0.02,
      );

  static TextStyle headlineMedium({
    double? tablet,
    double? ultraTablet,
  }) =>
      GoogleFonts.manrope(
        fontSize: Dimens.d24.responsive(tablet: tablet, ultraTablet: ultraTablet),
        fontWeight: FontWeight.w600,
        color: AppColors.current.primaryTextColor,
        letterSpacing: -0.02,
      );

  // Plus Jakarta Sans for Body & Labels
  static TextStyle bodyLarge({
    double? tablet,
    double? ultraTablet,
  }) =>
      GoogleFonts.plusJakartaSans(
        fontSize: Dimens.d16.responsive(tablet: tablet, ultraTablet: ultraTablet),
        fontWeight: FontWeight.w400,
        color: AppColors.current.primaryTextColor,
        letterSpacing: 0,
      );

  static TextStyle bodySmallVariant({
    double? tablet,
    double? ultraTablet,
  }) =>
      GoogleFonts.plusJakartaSans(
        fontSize: Dimens.d12.responsive(tablet: tablet, ultraTablet: ultraTablet),
        fontWeight: FontWeight.w400,
        color: AppColors.current.secondaryTextColor,
        letterSpacing: 0,
      );

  static TextStyle labelSmall({
    double? tablet,
    double? ultraTablet,
  }) =>
      GoogleFonts.plusJakartaSans(
        fontSize: Dimens.d10.responsive(tablet: tablet, ultraTablet: ultraTablet),
        fontWeight: FontWeight.w500,
        color: AppColors.current.tertiaryColor,
        letterSpacing: 0.05,
      );
      
  static TextStyle button({
    double? tablet,
    double? ultraTablet,
  }) =>
      GoogleFonts.manrope(
        fontSize: Dimens.d16.responsive(tablet: tablet, ultraTablet: ultraTablet),
        fontWeight: FontWeight.w600,
        color: AppColors.current.primaryTextColor,
        letterSpacing: 0,
      );

  static TextStyle headlineSmall({
    double? tablet,
    double? ultraTablet,
  }) =>
      GoogleFonts.manrope(
        fontSize: Dimens.d18.responsive(tablet: tablet, ultraTablet: ultraTablet),
        fontWeight: FontWeight.w600,
        color: AppColors.current.primaryTextColor,
        letterSpacing: -0.01,
      );

  static TextStyle bodyMedium({
    double? tablet,
    double? ultraTablet,
  }) =>
      GoogleFonts.plusJakartaSans(
        fontSize: Dimens.d14.responsive(tablet: tablet, ultraTablet: ultraTablet),
        fontWeight: FontWeight.w400,
        color: AppColors.current.secondaryTextColor,
        letterSpacing: 0,
      );
}
